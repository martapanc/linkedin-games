...
pnpm --filter queens-pwa dev      # play
pnpm --filter queens-pwa bench    # generator sanity-check + difficulty calibration
```

Or `cd apps/queens` first and drop the filter.
...
apps/queens/
  ...
  scripts/bench.ts
packages/core/    the difficulty ladder, seeded RNG, localStorage, leaderboard
                  rules, and the dialogs both games share
```
