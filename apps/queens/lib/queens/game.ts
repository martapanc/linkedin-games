...
export function getHint(puzzle: Puzzle, marks: Mark[]): Hint {
    ...
    for (let i = 0; i < marks.length; i++) {
        if (marks[i] === 2 && !truth.has(i)) {
            return {
                title: "Mistake",
                text: "This queen can't be right — anything you work out from it will be off too.",
                evidence: [],
                targets: [i],
                action: null,
            };
    ...
    return {
        title: "Queen",
        text: "No single step forces the next move — but a queen goes here.",
        evidence: [],
        targets: [remaining[0]],
        action: "place",
    };
