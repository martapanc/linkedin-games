import { regionName } from "./palette";
  ...
  for (const [cellsOf, doneOf, kind] of UNIT_GROUPS) {
    ...
    for (let k = 0; k < u.n; k++) {
      ...
      return {
        ...
        text: `${sentence(unitName(kind, k))} has only one square left, so its queen must go on ${cellName(u.n, cs[0])}.`,
  ...
  for (let g = 0; g < u.n; g++) {
    ...
    for (const [of, lineCells, label] of axes) {
      ...
      return {
        ...
        text: `Every square still open in ${unitName("reg", g)} sits in ${label} ${v + 1}. So that ${label}'s queen has to come from ${unitName("reg", g)} — the rest of ${label} ${v + 1} can be crossed out.`,
        evidence: cs,
        targets,
        action: "cross",
      };
  ...
  for (const [cells, done, kind] of lines) {
    for (let k = 0; k < u.n; k++) {
      ...
      return {
        rule: "locked",
        title: RULE_TITLE.locked,
        text: `Every square still open in ${unitName(kind, k)} lies inside ${unitName("reg", g0)}. So that area's queen has to be in ${unitName(kind, k)} — its squares elsewhere can be crossed out.`,
        evidence: cs,
        targets,
        action: "cross",
      };
    ...
    for (let k = 0; k < u.n; k++) {
      ...
      for (let x = 0; x < u.n * u.n; x++) {
        ...
        return {
          ...
          text: `A queen on ${cellName(u.n, x)} would knock out every square ${unitName(kind, k)} has left, leaving it nowhere to go. So ${cellName(u.n, x)} can be crossed out.`,
          evidence: cs,
          targets: [x],
          action: "cross",
        };
...
function findHall(u: Units, s: State): Deduction | null {
  ...
  for (const [xk, yk] of pairs) {
    ...
    const attempt = (group: number[]): Deduction | null => {
      ...
      const xs = joinList(group.map((g) => unitName(xk, g)));
      const yl = joinList(ys.map((y) => unitName(yk, y)));
      return {
        rule: "hall",
        title: RULE_TITLE.hall,
        text: `Between them, ${xs} can only use ${yl}. That uses ${yl} up, so every other square there can be crossed out.`,
        evidence,
        targets,
        action: "cross",
      };
...
function findTrial(u: Units, s: State): Deduction | null {
  for (let x = 0; x < u.n * u.n; x++) {
    ...
    return {
      ...
      text: `Try a queen on ${cellName(u.n, x)} and follow it through — a row, column or area ends up with no squares left. So ${cellName(u.n, x)} can be crossed out.`,
      evidence: [],
      targets: [x],
      action: "cross",
    };
